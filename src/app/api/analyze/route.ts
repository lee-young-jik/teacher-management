...
// AssemblyAI 클라이언트 (lazy 초기화 - 빌드 시 에러 방지)
  ...
  try {
    ...
    if (!allowedVideoTypes.includes(video.type)) {
      return NextResponse.json({ 
        error: `지원하지 않는 파일 형식입니다. (MP4, MOV, AVI 등만 가능) 현재: ${video.type}` 
      }, { status: 400 });
    }
    ...
    // ===== 1단계: 비디오 → AssemblyAI 업로드 (메모리에서 직접) =====
    const videoBuffer = Buffer.from(await video.arrayBuffer());
    console.log(`📹 영상 처리 중: ${video.name} (${(video.size / 1024 / 1024).toFixed(2)}MB)`);

    const aai = getAssemblyAI();
    
    console.log('📤 AssemblyAI에 비디오 업로드 중...');
    const uploadResponse = await aai.files.upload(videoBuffer);
    console.log('✅ AssemblyAI 업로드 완료:', uploadResponse);

    // ===== 2단계: 트랜스크립션 =====
    console.log('🎙️ 트랜스크립션 시작...');
    ...
    console.log(`🎤 화자 구분 결과: 총 ${transcript.utterances?.length || 0}개 발화`);
...
주의: 위 대화 내용은 영어로 음성인식된 결과이지만, 실제로는 한국어 수업 상황입니다. 맥락을 고려하여 분석해주세요.`
    ...
    // ===== 4단계: 결과 저장 (Supabase만 사용 - 파일시스템 없음) =====
    if (analysisResponse.choices[0].message?.content) {
      ...
      try {
        if (transcript.utterances && transcript.utterances.length > 0) {
          ...
          console.log(`📏 재생 시간: ${videoDuration}`);
        }
      } catch (e) {
        console.warn('재생 시간 추출 실패');
      }
      ...
      try {
        ...
        console.error('❌ Supabase 예외:', supabaseErr);
      }
    }
